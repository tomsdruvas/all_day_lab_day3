...
8. Create class diagrams for every class
9. Create object diagrams
...
### MVP:
  ...
  - A `Drink` should have a `name`,`price` and an `alcoholic status`
  - A `Customer` should have a `name`, a `stomach?` and a `wallet`
  - A `Customer` should be able to buy a `Drink` from the `Pub`, reducing the money in its `wallet` and increasing the money in the `Pub`'s `till`. The customer should be able to recieve the drink from the pub. 

### Extensions:

  - Add an `age` to the `Customer`. Make sure the `Pub` checks the `age` before serving the `Customer`. If the customer is under age the pub can only offer a soft drink option. 
...
- Pub should be able to calculate their stock's `drink` price
- Pub could add a mark up value to create a profit when they are selling. 
